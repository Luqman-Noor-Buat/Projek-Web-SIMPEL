...
//subkegiatan
$id = $_GET["id"];
$sql = "SELECT * FROM subkegiatan WHERE ID_KEGIATAN='$id'";
...
if(isset($_POST["cari"])) {
    ...
    if(mysqli_affected_rows($conn) < 0) {
        ...
        $tampil2 = false;
        //menu edit
        $sql4 = "SELECT * FROM transaksi WHERE ID_TEMA = $id_tema AND ID_KEGIATAN='$id'";
        $hasil4 = mysqli_query($conn, $sql4);
        $data_menu_edit = [];
        while ($row4 = mysqli_fetch_array($hasil4)){
            $id_subkegiatan = $row4['ID_SUBKEGIATAN'];
            $sql5 = "SELECT * FROM transaksi WHERE ID_TEMA = $id_tema AND ID_SUBKEGIATAN='$id_subkegiatan'";
            $hasil5 = mysqli_query($conn, $sql5);
            $row5 = mysqli_fetch_array($hasil5);
            $menu_edit = $row5['KECAMATAN'];    
            if ($menu_edit == NULL){
                $tampil_menu_edit1 = true;
                $data_menu_edit[] = $menu_edit; 
            }else{
                $data_menu_edit[] = $menu_edit; 
            }
        }
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card-style mb-30">
                                            ...
                                            <?php while($row3 = mysqli_fetch_array($hasil3)) : ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <?php if(isset($tampil_menu_edit1) && $row3['KECAMATAN']==$data_menu_edit[$urutan] && $row3['KECAMATAN']==NULL) : ?>
                                                            <a href="edit.php?id=<?=$row3['ID_SUBKEGIATAN'],$id_tema?>"><button class="btn btn-danger">&nbsp;Edit&nbsp;</button></a>
                                                        <?php endif; ?>
                                                        <?php $urutan ++;?>
                                                    </td>
                                                </tr>
                                ...
                                <!-- end table wrapper -->
                                <?php if ($row == NULL) :?>
                                    <p style="color: red;">* Subkegiatan kosong.</p>
                                <?php else: ?>
                                    <a href="edit_subkegiatan.php?id=<?=$id?>"><button class="btn btn-primary">Edit Subkegiatan</button></a>
                                <?php endif; ?>
                            </div>
                            <!-- end card -->
                        </div>
